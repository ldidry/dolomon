...
sub run {
    ...
    unless (-d $home) {
        ...
        make_path(File::Spec->catdir($home, 'lib', 'Dolomon', 'I18N'));
...
EOF
        ...
        open my $f, '>', File::Spec->catfile($home, 'lib', 'Dolomon', 'I18N.pm') or die "Unable to open $home/lib/Dolomon/I18N.pm: $!";
        print $f $i18n;
        close $f;
...
SEDOPTS=-e "s\@SOME DESCRIPTIVE TITLE\@Dolomon language file\@" \\
		...
		-e "s\@CHARSET\@utf8\@" \\
		-e "s\@the PACKAGE package\@the Dolomon package\@" \\
		-e '/^\\#\\. (/{N;/\\n\\#\\. (/{N;/\\n.*\\.\\.\\/default\\//{s/\\#\\..*\\n.*\\#\\./\\#. (/g}}}' \\
		-e '/^\\#\\. (/{N;/\\n.*\\.\\.\\/default\\//{s/\\n/ /}}' 
...
Dolomon::Command::theme - Create new theme skeleton.

=head1 SYNOPSIS

  Usage: script/dolomon theme THEME_NAME
